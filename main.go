...
func main() {
	...
	page4, cursor, err := selectNextPage(ctx, db, cursor.End)
	if err != nil {
		panic(err)
	}
...
func getWindow(entries []Entry) ([]Entry, Cursor, error) {
	if len(entries) > pageLength {
		return entries[0:pageLength], NewCursor(entries), nil
	} else {
